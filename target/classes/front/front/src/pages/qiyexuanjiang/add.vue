<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="成立时间" prop="chenglishijian">
				<el-date-picker
					:disabled=" false  ||ro.chenglishijian"
			...
			<el-form-item class="add-item" label="企业宣言" prop="qiyexuanyan">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.farendaibiao!=''&&json.farendaibiao) || json.farendaibiao==0){
			...
			// 多级联动参数
			info() {
				this.$http.get(`qiyexuanjiang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`qiyexuanjiang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
