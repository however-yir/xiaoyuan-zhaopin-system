	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="投递日期" prop="toudiriqi" >
					<el-input v-model="ruleForm.toudiriqi" placeholder="投递日期" clearable  :readonly="ro.toudiriqi"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="投递日期" prop="toudiriqi" >
				...
				<el-form-item class="input" v-else-if="ruleForm.yaoqingshijian" label="邀请时间" prop="yaoqingshijian" >
			...
			<el-form-item class="textarea" v-if="type!='info'" label="应聘须知" prop="yingpinxuzhi" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.yingpinxuzhi" label="应聘须知" prop="yingpinxuzhi" >
				<span class="text">{{ruleForm.yingpinxuzhi}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yingyaozhaopin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yingyaozhaopinCrossAddOrUpdateFlag = false;
											this.parent.search();
