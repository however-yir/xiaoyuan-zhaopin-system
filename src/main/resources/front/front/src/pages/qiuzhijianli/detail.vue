	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/qiuzhijianli?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">教育经历</div>
						<div class="text "  >{{detail.jiaoyujingli}}</div>
					</div>
					<div class="item">
						<div class="lable">培训经历</div>
						<div class="text "  >{{detail.peixunjingli}}</div>
					</div>
					<div class="item">
						<div class="lable">经历实践</div>
						<div class="text "  >{{detail.jinglishijian}}</div>
					</div>
					<div class="item">
						<div class="lable">语言能力</div>
						<div class="text "  >{{detail.yuyannengli}}</div>
					</div>
					<div class="item">
						<div class="lable">自我评价</div>
						<div class="text "  >{{detail.ziwopingjia}}</div>
					</div>
					<div class="item">
						<div class="lable">求职意向</div>
						<div class="text "  >{{detail.qiuzhiyixiang}}</div>
					</div>
					<div class="item">
						<div class="lable">获得证书</div>
						<div class="text "  >{{detail.huodezhengshu}}</div>
					</div>
					<div class="item">
						<div class="lable">投递日期</div>
						<div class="text "  >{{detail.toudiriqi}}</div>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('qiuzhijianli','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('qiuzhijianli','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('qiuzhijianli','招聘')" 
							@click="onAcross('yingyaozhaopin','是','','','')" type="warning">
							招聘
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('qiuzhijianli','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`qiuzhijianli`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/qiuzhijianli', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/qiuzhijianliAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此求职简历？') .then(_ => {
					this.$http.post('qiuzhijianli/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
