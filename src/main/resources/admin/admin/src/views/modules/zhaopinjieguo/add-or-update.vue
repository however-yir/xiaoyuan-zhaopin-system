<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="面试须知" prop="mianshixuzhi" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="面试须知"
					v-model="ruleForm.mianshixuzhi" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.mianshixuzhi" label="面试须知" prop="mianshixuzhi" >
				<span class="text">{{ruleForm.mianshixuzhi}}</span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='mianshixuzhi'){
							this.ruleForm.mianshixuzhi = obj[o];
							this.ro.mianshixuzhi = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zhaopinjieguo/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zhaopinjieguoCrossAddOrUpdateFlag = false;
											this.parent.search();
